...
import { IGroupConfig, IMultiSelectFacet } from './models';
...
describe('MultiSelectFacet', () => {
  describe('Groups are on the same level', () => {
    const flatGroupConfig = [{ name: 'name' }, { name: 'age' }, { name: 'height' }] as IGroupConfig[];
    ...
    it('returns filtered result', async () => {
      ...
      await expectAsync(sut.result.pipe(take(1)).toPromise()).toBeResolvedTo([
        { name: 'Tom', age: 35, height: 225 },
        { name: 'Tom', age: 38, height: 185 },
      ]);
    });
    ...
    it('returns result on each filterByGroup', async () => {
      ...
      await expectAsync(sut.result.pipe(take(1)).toPromise()).toBeResolvedTo([{ name: 'Tom', age: 38, height: 185 }]);
    });
    ...
    it('doesnt filter available items in same level groups on filterByGroup', async () => {
      ...
      await expectAsync(sut.groups.height.pipe(take(1)).toPromise()).toBeResolvedTo([
        { name: 'Joe', age: 17, height: 125 },
        { name: 'Bob', age: 17, height: 221 },
        { name: 'Tom', age: 35, height: 225 },
        { name: 'Tom', age: 38, height: 185 },
